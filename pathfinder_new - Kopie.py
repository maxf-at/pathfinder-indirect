...
class Fp_class():
        ...
        while (current_bp != current_bp_end):
            ...
            for current_path in paths:
                ...
                if pos == done:
                    # print ("undo")
                    for i, j in add_moves:
                        if i < 0 and current_p_table[-i] == -j:
                            alt = True
                            current_p_table[-i] = 0
                            current_p_table[-j] = 0
                    ...
                    for i, j in add_moves:
                        ...
                        if i > 0 and current_p_table[i] == 0 and current_p_table[j] == 0 and temp_loop[i] == temp_loop[j]:
    ...
    def print_moves(self, sequence, s1, s2, moves, indirect_moves, move_color='\033[93m', Verbose=True, ignore_moves=set()):
        ...
        for a, move in enumerate(moves):
            ...
            # del all indirect moves first
            for x,y in indirect_moves[a]:
                if x > 0:
                    pt[x] = 0
                    pt[y] = 0
